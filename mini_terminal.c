...
/*void pidredir(char* aaaa,int co){
		FILE *fd2;
		fd2=fopen(aaaa,"wb");
		//fprintf(fd2,"%s","This is just an example :)\n");
		if(co==1){
			pid_t p;
			p=getpid();
			fprintf(fd2,"command name: ./a.out process id: %d\n",p);
		}
		else{
			if(strcmp(comm[1],"current")==0){                   //pid current
				fprintf(fd2,"%s","List of currently executing processes spawned from this shell:\n");
				int i;
				for(i=1;i<allp;i++){
					if (kill(pidall[i], 0) == 0){
						fprintf(fd2,"command name: %s process id: %d\n",all[i], pidall[i]);
						// process is running or a zombie 
			...
			if(strcmp(comm[1],"all")==0){                     //pid all
				fprintf(fd2,"%s","List of all processes spawned from this shell:\n");
				int i;
				for(i=1;i<allp;i++){
					fprintf(fd2,"command name: %s process id: %d\n",all[i], pidall[i]);
				}
